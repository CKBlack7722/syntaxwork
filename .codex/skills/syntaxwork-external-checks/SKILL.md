...
8. Allow `條件` to remain blank. Generate active SPSS syntax only from rows where `m`, `p`, and `條件` are present; blank `條件` rows may be emitted as commented skeletons for user completion.
