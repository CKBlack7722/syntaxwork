**3.複選題檢核.
* K1 K2 KFB3 KFB4 KIG3 KIG4 KTT3 KTT4 KTT5 K3 E2 E13 G7 G8 G9 G10 G11 B7a B8 G3 F5 C3 JB1 JB2 M2 M3 M4s Q20 Q25 Q22 Q23 Q26 Q27 Q30 Q31 ZE2 組合.
STRING K1 K2 KFB3 KFB4 KIG3 KIG4 KTT3 KTT4 KTT5 K3 E2 E13 G7 G8 G9 G10 G11 B7a B8 G3 F5 C3 JB1 JB2 M2 M3 M4s Q20 Q25 Q22 Q23 Q26 Q27 Q30 Q31 ZE2 (A600).
...
COMPUTE M4s = Rtrim(Ltrim(concat("vM4sM4=",string(vM4sM4,f2)," , ","vM4sM5=",string(vM4sM5,f2)," , ","vM4sM6=",string(vM4sM6,f2)))).
...
compute p1126="M3至少選1項".
end if.
end loop.
exec.

*M4s.
vector a=vM4sM4 to vM4sM6.
COMPUTE #M4szero = (SUM(vM4sM4 TO vM4sM6) = 0).
loop #i=1 to 2.
do if #M4szero=1.
compute m1127=Rtrim(Ltrim(M4s)).
compute p1127="M4s至少選1項".
end if.
end loop.
exec.
...
| (any(a(#i),90) and not any(a(#i+1),90))
| #Q20zero=1.
compute m1128=Rtrim(Ltrim(Q20)).
compute p1128="Q20至少選1項或選特殊碼應一致".
end if.
end loop.
exec.
...
loop #i=1 to 31.
do if #Q25zero=1.
compute m1129=Rtrim(Ltrim(Q25)).
compute p1129="Q25至少選1項".
end if.
end loop.
exec.
...
loop #i=1 to 40.
do if #Q22zero=1.
compute m1130=Rtrim(Ltrim(Q22)).
compute p1130="Q22至少選1項".
end if.
end loop.
exec.
...
loop #i=1 to 27.
do if #Q23zero=1.
compute m1131=Rtrim(Ltrim(Q23)).
compute p1131="Q23至少選1項".
end if.
end loop.
exec.
...
loop #i=1 to 29.
do if #Q26zero=1.
compute m1132=Rtrim(Ltrim(Q26)).
compute p1132="Q26至少選1項".
end if.
end loop.
exec.
...
loop #i=1 to 7.
do if #Q27zero=1.
compute m1133=Rtrim(Ltrim(Q27)).
compute p1133="Q27至少選1項".
end if.
end loop.
exec.
...
| (any(a(#i),90) and not any(a(#i+1),90))
| #Q30zero=1.
compute m1134=Rtrim(Ltrim(Q30)).
compute p1134="Q30至少選1項或選特殊碼應一致".
end if.
end loop.
exec.
...
| (any(a(#i),90) and not any(a(#i+1),90))
| #Q31zero=1.
compute m1135=Rtrim(Ltrim(Q31)).
compute p1135="Q31至少選1項或選特殊碼應一致".
end if.
end loop.
exec.
...
loop #i=1 to 2.
do if #ZE2zero=1.
compute m1136=Rtrim(Ltrim(ZE2)).
compute p1136="ZE2至少選1項".
end if.
end loop.
exec.
